...
Run this document locally with:

```r
rmarkdown::run("vignettes/aggregated-summary.Rmd")
```
...
```{r annotator-super-simple, echo = FALSE}
local({
  queue <- reactive({
    data.frame(
      id      = 1:6,
  ...
  shinyApp(ui, server, options = list(height = 480))
})
```
...
```{r milestone-tracker, echo = FALSE}
local({
  milestones <- reactive({
    data.frame(
      id         = 1:8,
