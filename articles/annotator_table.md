...
The code below is the full self-contained app used in the live demo.
Copy it into a new R session and run `shinyApp(ui, server)` to try it
interactively. It uses a representative set of developmental milestone
rows: check **Met?** to mark a milestone as achieved, and add a short
note — the annotation panel updates in real time to show only rows you
have touched.
